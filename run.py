...
def main() -> None:
    ...
    df_2019 = transformer.transform(dfs[7], 2019)
    df_2017 = transformer.transform(dfs[7], 2017)
    df_2013 = transformer.transform(dfs[7], 2013)
    df_2011 = transformer.transform(dfs[7], 2011)
    df_2011_2020 = pd.concat([df_2011,df_2013,df_2017,df_2019,df_2020])

    writer.write_to_postgres(df=df_2011_2020, db_name=DB_NAME, table_name=TABLE_NAME, connection_string=CONNECTION_STRING)
